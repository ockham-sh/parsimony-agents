"""Broker ↔ socket-transport round-trip: results cross, secrets don't, errors reconstruct.

Exercised over an in-memory socketpair (no subprocess): the kernel-side transport
RPCs the supervisor-side broker, which runs the bound connector and returns the
Result. Validates the security-critical invariants of the broker channel.
"""
...
from parsimony_agents.execution.sandbox.connector_rpc import ConnectorBroker, SocketConnectorTransport
...
async def _ok_fetch(series_id: str, api_key: str) -> pd.DataFrame:
...
async def _bad_fetch(series_id: str, api_key: str) -> pd.DataFrame:
...
async def test_broker_runs_connector_and_returns_tabular() -> None:
    ...
    try:
        transport = SocketConnectorTransport("client", ker)
        result = await transport.invoke("ok_fetch", (), {"series_id": "GDPC1"})
...
async def test_broker_marshals_connector_error_as_typed() -> None:
    ...
    try:
        ...
        with pytest.raises(ConnectorError) as ei:
            await transport.invoke("bad_fetch", (), {"series_id": "X"})
...
async def test_broker_rejects_ungranted_binding() -> None:
    ...
    sup, ker = await _wire({"client": bound})
    try:
        transport = SocketConnectorTransport("other_binding", ker)
        with pytest.raises(RuntimeError) as ei:
            await transport.invoke("ok_fetch", (), {"series_id": "GDPC1"})
...
async def test_broker_rejects_unknown_connector_name() -> None:
    ...
    try:
        ...
        with pytest.raises(RuntimeError):
            await transport.invoke("nonexistent_fetch", (), {})
    finally:
        await sup.close()
        await ker.close()
...
@pytest.mark.asyncio
async def test_transport_rejects_non_json_args_with_named_error() -> None:
    ...
    try:
        ...
        with pytest.raises(TypeError) as ei:
            await transport.invoke("ok_fetch", (), {"series_id": datetime(2020, 1, 1)})
        msg = str(ei.value)
