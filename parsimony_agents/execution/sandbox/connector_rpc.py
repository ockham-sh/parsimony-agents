"""One connector call crossing the kernel boundary: codec, broker, stub.

The kernel-side :class:`RemoteConnector` sends ``connector_invoke`` over the
duplex RPC; the supervisor-side :class:`ConnectorBroker` — which holds the
**bound** connectors, so credentials live only in the trusted process — runs
``connector.fn`` and returns the framework-built :class:`Result`. The broker is
the only component that performs connector network egress, making a bound
connector the sole outbound path once the kernel substrate denies everything
else.

The kernel never receives a bound connector; it receives only the connector's
name and a socket — connector metadata (cards, parameters) is the agent loop's
concern, rendered host-side into the prompt, never shipped into the kernel. The
shared codec is both halves' wire language: a tabular result crosses as
...
class ConnectorBroker:
    """Runs bound connectors on behalf of the kernel's name-only stubs.
...
class RemoteConnector:
    """Kernel-side stand-in for one bound connector: a name plus a socket.

    What the agent's namespace holds in place of a credentialed
    :class:`~parsimony.connector.Connector`. It carries only the connector's
    name — no ``fn``, ``bound_arguments``, ``secrets``, and no metadata (cards
    and parameters are the agent loop's concern, rendered host-side, never
    shipped into the kernel). Calling it sends a ``connector_invoke`` over the
    supervisor connection to the broker, which runs the real connector. This is
    the kernel half of "a bound connector is the only way out": the stub opens no
    network socket of its own and holds no credential.
    """

    __slots__ = ("name", "_binding", "_rpc", "_loop")

    def __init__(self, name: str, binding: str, rpc: RpcEndpoint, loop: asyncio.AbstractEventLoop) -> None:
        self.name = name
    ...
    def __call__(self, *args: Any, **kwargs: Any) -> Result:
        """Synchronous call — matches the in-process connector contract.

        Agent code calls connectors synchronously (``connectors["x"](...)``,
        no ``await``); under the sandbox that code runs in the executor's worker
        thread, so we drive the async RPC on the kernel's event loop and block
        the thread until it returns. The kernel owns no credential: the call
        round-trips to the broker in the trusted supervisor.
        """
        _check_json_args(self.name, args, kwargs)
        fut = asyncio.run_coroutine_threadsafe(
            self._rpc.call(
                "connector_invoke",
                {"binding": self._binding, "name": self.name, "args": list(args), "kwargs": dict(kwargs)},
            ),
            self._loop,
        )
        resp, blob = fut.result()
