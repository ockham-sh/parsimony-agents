...
connectors — injects :class:`ConnectorProxy` bundles whose calls RPC back to the
supervisor's broker. The kernel holds no credential; a connector reaches the
network only by that callback.
"""
...
from parsimony.capability import ConnectorManifest
...
from parsimony_agents.execution.sandbox.connector_rpc import SocketConnectorTransport
...
def _make_remote_binder(
    rpc: RpcEndpoint,
    bindings: dict[str, list[ConnectorManifest]],
) -> Callable[[ConnectorCache, tuple[Callable[..., Any], ...]], dict[str, Any]]:
    """A binder that produces proxy bundles whose transport RPCs the broker."""

    def binder(cache: ConnectorCache, post_hooks: tuple[Callable[..., Any], ...]) -> dict[str, Any]:
        out: dict[str, Any] = {}
        for binding, manifests in bindings.items():
            transport = MemoizingConnectorTransport(SocketConnectorTransport(binding, rpc), cache, post_hooks)
            out[binding] = proxy_bundle(manifests, transport)
        return out
...
class _CommandHandler:
    ...
    async def _cmd_set_connectors(self, params: dict[str, Any], blob: bytes) -> tuple[dict[str, Any], bytes]:
        ...
        bindings = {
            binding: [ConnectorManifest.from_wire(m) for m in manifests]
            for binding, manifests in params.get("bindings", {}).items()
        }
        await self._ex._set_remote_connectors(_make_remote_binder(self._rpc, bindings))
        return {}, b""
