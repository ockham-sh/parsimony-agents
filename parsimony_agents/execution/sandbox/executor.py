...
supervisor; the kernel receives only secret-free manifests.
"""
...
class SandboxedCodeExecutor(BaseCodeExecutor):
    ...
    def __init__(self, *, cwd: str, confine: bool = False, scratch_root: str | None = None) -> None:
        ...
        # The last bound bundles, kept to re-ship manifests when a dead kernel
        # is replaced (the broker keeps serving; the new kernel needs proxies).
    ...
    async def _restore_kernel_state(self) -> None:
        """Re-prime a respawned kernel: connector manifests + setup snippets.

        A kernel that died took its namespace with it; what the supervisor can
        restore is the connector grant (manifests re-shipped from the held
        ...
        if self._bundles:
            wire = {binding: [c.to_manifest().to_wire() for c in bundle] for binding, bundle in self._bundles.items()}
    ...
    async def _call(
        ...
        *,
        ...
        try:
            ...
            # (manifests + setup snippets restored), and surface one clear
    ...
    async def set_connectors(self, connectors: Any) -> None:
        """Hold the bound connectors here; ship only secret-free manifests to the kernel."""
        await self._ensure_started()
        bundles = normalize_connector_bundles(connectors)
        self._bundles = bundles
        self._broker.set_bundles(bundles)
        wire = {binding: [c.to_manifest().to_wire() for c in bundle] for binding, bundle in bundles.items()}
