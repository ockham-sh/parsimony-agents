    ...
    **Power usage (product / full control):**
    ...
    @toolmethod(
        ...
        description=(
            ...
            "  - Default to prose, charts, and callouts. The report is narrative; "
            "the reader has the full dataset one click away in the workspace "
            "artifact panel. Don't reproduce data in the report that already "
            "lives in the workspace.\n"
            "  - Small comparison tables (≤ ~6 rows × ≤ ~4 columns, every cell "
            "genuinely meaningful side-by-side: scenario A vs B vs C, before/after, "
            "pros/cons) are fine as hand-authored markdown tables.\n"
            "  - For trends, distributions, or shape-of-the-data, the answer is a "
            "CHART, not a table. Embed a chart with "
            "![Caption](file://./.ockham/charts/<logical_id>/<content_sha>.vl.json).\n"
            "  - Embed a dataset PREVIEW only when the reader needs to see actual "
            "cell values — categorical breakdowns, top/bottom-N rows, first/last "
            "entries of a time series. Use the same image syntax as a chart: "
            "![Caption](file://./.ockham/datasets/<logical_id>/<content_sha>.parquet) "
            "(the `![…]` form, NOT the `[…]` link form). The renderer inlines a "
            "10-row preview by default with a 'Showing N of M rows' footer — you "
            "don't need to narrate the row count in prose. Override with the "
            "pandoc attribute {preview-rows=N} (clamped 1–100) only when a specific "
            "window is meaningful, e.g. ![Top 20 issuers](file://...){preview-rows=20}.\n"
            "  - If a long table is genuinely the best representation (audit log, "
            "row-by-row reconciliation) say so explicitly in the surrounding prose, "
            "use the {preview-rows=N} attribute to set the window, and keep it to "
            "one such table per report."
        ),
        parameters_schema={
            "type": "object",
            "properties": {
