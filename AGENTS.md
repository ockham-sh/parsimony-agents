...
- A standalone `Agent` uses the in-process `CodeExecutor` by default, with no isolation boundary. Hosts can inject `create_executor(...)`; on supported Linux systems it selects a bwrap-confined KERNEL process that reaches credentialed connectors through a BROKER over RPC, and otherwise falls back in-process with a warning. `SandboxedCodeExecutor(confine=False)` is process separation only, not confinement. Check `capability_tier` before treating execution as isolated.
