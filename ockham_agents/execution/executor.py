...
from ockham_agents.theme import register_theme
...
class CodeExecutor(BaseCodeExecutor):
    ...
    def _apply_connectors(self) -> None:
        ...
        if self._connectors is None:
            return
        from ockham_agents.execution.fetch_log import make_fetch_logger
            ...
            return KernelOutput(
                outputs=[
                    self._output_factory.from_value(
                        RuntimeError("duckdb is not installed; install ockham-agents with the [sql] extra.")
                    )
                ]
            )
        with self._lock:
